...
const TRIGGERS = [
  { value: 'webhook', label: '🌐 Webhook (chamada HTTP)' },
  { value: 'schedule', label: '🕐 Agendamento (cron)' },
  { value: 'manual', label: '▶️ Manual (botão)' },
  { value: 'event', label: '⚡ Evento (mensagem recebida)' },
];

const ACTIONS = [
  { value: 'message', label: '💬 Enviar mensagem' },
  { value: 'discord', label: '🎮 Postar no Discord' },
...
const EXAMPLES = [
  {
    ...
    enabled: true, runs: 12,
  },
  {
    name: 'Suporte Automático',
    description: 'Responde mensagens de clientes automaticamente',
    steps: [
  ...
  {
    name: 'Post Instagram',
    description: 'Cria e posta conteúdo para o Instagram toda semana',
    steps: [
      { type: 'trigger' as const, label: '🕐 Segunda, 10h' },
      { type: 'agent' as const, label: '📣 Copywriter' },
      { type: 'action' as const, label: '📸 Postar no Instagram' },
    ],
    enabled: false, runs: 3,
  },
];
...
export function WorkflowsPage() {
  ...
  async function run(id: string) {
    await api.post(`/workflows/${id}/run`, {}).catch(() => alert('Erro ao executar workflow'));
            ...
            {EXAMPLES.map((ex, i) => (
              ...
              >
                ...
                <div className="workflow-meta">
                  <span className="badge badge-gray">{ex.runs} execuções</span>
        ...
        <div className="modal-overlay" onClick={() => setShowCreate(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ width: 560 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div className="form-group">
                <label>Config do Trigger</label>
                <input
                  value={form.triggerConfig}
                  onChange={e => setForm(f => ({ ...f, triggerConfig: e.target.value }))}
                  placeholder={form.triggerType === 'schedule' ? '0 9 * * *' : form.triggerType === 'webhook' ? '/meu-webhook' : ''}
                />
              </div>
            </div>

            <div className="form-group">
              <label>Agentes (IDs separados por vírgula)</label>
              <input
                value={form.agentIds}
                onChange={e => setForm(f => ({ ...f, agentIds: e.target.value }))}
                placeholder="IDs dos agentes que vão executar"
              />
              <small>Deixe vazio para usar o assistente geral</small>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div className="form-group">
                <label>Config da Ação</label>
                <input
                  value={form.actionConfig}
                  onChange={e => setForm(f => ({ ...f, actionConfig: e.target.value }))}
                  placeholder={form.actionType === 'discord' ? 'Webhook URL do Discord' : form.actionType === 'webhook' ? 'URL do webhook' : ''}
                />
              </div>
            </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div className="pipeline">
                  <div className="pipeline-node trigger">
                    {TRIGGERS.find(t => t.value === form.triggerType)?.label.split('(')[0].trim() || 'Trigger'}
                  </div>
                  <span className="pipeline-arrow">→</span>
                  <div className="pipeline-node agent-node">
                    {form.agentIds ? '🤖 Agentes' : '✨ Assistente Geral'}
                  </div>
                  <span className="pipeline-arrow">→</span>
                  <div className="pipeline-node action">
                    {ACTIONS.find(a => a.value === form.actionType)?.label.split('(')[0].trim() || 'Ação'}
                  </div>
                </div>
              </div>
            )}
